#!/usr/bin/env python3
"""
Main orchestrator for MobileDokan scraper.

This module provides the MobileDokanScraper class that coordinates
the complete scraping workflow with comprehensive error handling
    ...
    def _extract_phone_urls_limited(self, base_url: str, max_phones: int) -> List[dict]:
        ...
        while len(phone_urls) < max_phones:
            ...
            try:
                ...
                # Use the client's backoff-aware path here as this optimized
                # branch performs its own page-by-page requests.
                response = self.http_client.get_with_backoff(page_url)
    ...
    def _scrape_phone_details(self, phone_urls: List[dict], 
        ...
        for i, phone_info in enumerate(urls_to_process, 1):
            try:
                ...
                if phone_specs and phone_specs.validate():
